...
version: 0.1.0
tags: [twitter, x, social-media, cli, engagement]
agents: [thread-theo, reply-rex, trend-tara]
env:
  - AUTH_TOKEN: X/Twitter auth_token cookie (from browser DevTools > Application > Cookies > x.com)
  - CT0: X/Twitter ct0 cookie (from browser DevTools > Application > Cookies > x.com)
  - SWEETISTICS_API_KEY: (optional) Sweetistics API key for posting without cookies
install:
...
1. Open Chrome/Firefox and log into x.com
...
```bash
export AUTH_TOKEN="your-auth-token-cookie"
export CT0="your-ct0-cookie"
```
...
### Thread Theo — Brand voice, posting tweets & threads
```bash
# Post a brand tweet
bird tweet "Xerus just shipped agent teams. Your AI workforce coordinates itself now."

# Post a thread (reply chain to your own tweet)
bird tweet "Thread: Why AI agent teams matter"
bird reply <tweet-id> "1/ Traditional automation breaks at the handoff..."
bird reply <reply-id> "2/ Agent teams solve this with native coordination..."

# Check home timeline for inspiration
bird home --following -n 20 --json

# Monitor trending topics for timely content
bird news --ai-only -n 10
```

### Reply Rex — Engagement hunting & replies
```bash
# Search for relevant conversations to engage with
bird search "AI agents workflow automation" -n 20 --json

# Find mentions to respond to
bird mentions -n 15 --json

# Check specific user timelines for reply opportunities
bird user-tweets @target_handle -n 10 --json

# Reply to relevant tweets
bird reply <tweet-url> "Interesting take! We've seen similar patterns..."

# Follow key accounts
bird follow @relevant_account
```

### Trend Tara — Trend research & curation
```bash
# Get trending/news topics
bird news -n 20 --json
bird trending --with-tweets

# Search trending topics
bird search "AI coding assistant 2026" -n 20 --json

# Read viral tweets for analysis
bird read <tweet-url> --json

# Get full conversation threads
bird thread <tweet-url> --json --all

# Monitor competitor timelines
bird user-tweets @competitor -n 20 --json

# Check bookmarks for saved research
